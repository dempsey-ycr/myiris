...
func NewQueueCache(db, host, password string) Cache {
	var pool = &redis.Pool{
		MaxIdle:     2,
		MaxActive:   5,
		Wait:        true,
		...
		Dial: func() (redis.Conn, error) {
			...
			if err != nil {
				fmt.Println("queue dial failed", err)
				return nil, err
			}
			if len(password) > 0 {
				if _, err := c.Do("AUTH", password); err != nil {
					fmt.Println("queue auth failed", err)
					return nil, err
				}
			}
...
func NewRedisCache(db int, host, password string, defaultExpiration time.Duration) *Cache {
	...
	var pool = &redis.Pool{
		...
		Dial: func() (redis.Conn, error) {
			dialOptions := make([]redis.DialOption, 3)
			dialOptions[0] = redis.DialConnectTimeout(10 * time.Second)
			dialOptions[1] = redis.DialReadTimeout(8 * time.Second)
			dialOptions[2] = redis.DialWriteTimeout(10 * time.Second)

			c, err := redis.Dial("tcp", host, dialOptions...)
			if err != nil {
				logger.Error("redis dial connection error: ", err)
				return nil, err
			}
			if len(password) > 0 {
				if _, err := c.Do("AUTH", password); err != nil {
					logger.Error("redis auth password error: ", err)
					return nil, err
				}
			}
			// check with PING
	...
	logger.Debug("redis client init success...")
...
func (c Cache) Increment(key string, delta uint64) (uint64, error) {
	...
	// redis1 will auto create the key, and we don't want that. Since we need to do increment
	// ourselves instead of natively via INCRBY (redis1 doesn't support wrapping), we get the value
...
func (c Cache) Decrement(key string, delta uint64) (newValue uint64, err error) {
	...
	defer conn.Close()
	// Check for existance *before* increment as per the cache contract.
	// redis1 will auto create the key, and we don't want that, hence the exists call
	existed, err := exists(conn, key)
	if err != nil {
		return 0, err
	} else if !existed {
...
//Lpush redis1 queue model source value
...
func (c Cache) SmembersInt64(key string) ([]int64, error) {
	conn := c.pool.Get()
	defer conn.Close()
	raws, err := conn.Do("SMEMBERS", key)
	...
	return redis.Int64s(raws, err)
}

//SmembersInterface 返回集合中的所有的成员
//不存在的集合 key 被视为空集合
func (c Cache) SmembersObj(key string, ptrValue interface{}) error {
	conn := c.pool.Get()
	defer conn.Close()
	raws, err := conn.Do("SMEMBERS", key)
	if err != nil {
...
//Zrangebyscore 通过分数返回有序集合指定区间内的成员
func (c Cache) ZrangebyscoreForInt64(key string, begin int64, end int64) ([]int64, error) {
	conn := c.pool.Get()
	defer conn.Close()
	raws, err := conn.Do("ZRANGEBYSCORE", key, begin, end)
	if err != nil {
...
func (c Cache) Hincrby(key, field string, count int) (int, error) {
	conn := c.pool.Get()
	defer conn.Close()
...
psc := redis1.PubSubConn{c}
psc.Subscribe("example")
for {
    switch v := psc.Receive().(type) {
    case redis1.Message:
        fmt.Printf("%s: message: %s\n", v.Channel, v.Data)
    case redis1.Subscription:
...
func (c Cache) Subscribe(key string, exec func([]byte, error)) {
	var conn = c.pool.Get()
...
Ext:
	if err := psc.Subscribe(key); err != nil {
		fmt.Println("send subscribe error: ", err)
		return
	}
	...
	for {
		switch v := psc.Receive().(type) {
		case redis.Message:
			item, err := redis.Bytes(v.Data, nil)
		...
		case redis.Subscription:
			fmt.Printf("%s: %s %d,v:%v", v.Channel, v.Kind, v.Count, v)
		case error:
			_ = psc.Unsubscribe(key) // 取消订阅
			fmt.Println("====xx", v)
			goto Ext // 开启下一次订阅
		}
	}
...
//Publish 将信息发送到指定的频道
func (c Cache) Publish(key string, value interface{}) error {
	conn := c.pool.Get()
	defer conn.Close()
	b, err := Serialize(value)
...
// 			item, err1 := redis1.Bytes(raw, err)
// 			if err1 != nil {
// 				fmt.Println("redis1.Bytes error:%v", err1)
// 			}
...
//IncrBy 自增
func (c Cache) IncrBy(key string, delta int) (int, error) {
	conn := c.pool.Get()
	defer conn.Close()
	val, err := conn.Do("INCRBY", key, delta)
	...
	newValue, err := redis.Int(val, nil)
...
//DECRBY 自减
func (c Cache) DecrBy(key string, delta int) (int, error) {
	conn := c.pool.Get()
	defer conn.Close()

	val, err := conn.Do("DECRBY", key, delta)
